...
!!! info "No English tutorial yet — one in Farsi"
    ...
    **Made one, or thinking about it?** Open an issue or a PR and we will link it. A recording of a real install is worth more than any amount of prose, and it finds bugs the maintainers never hit — several fixes in [2.1.0](https://github.com/MotherofallVPNs/MoaV/blob/main/CHANGELOG.md) came from watching the Farsi one. Credit stays with the author; we link rather than re-host.
